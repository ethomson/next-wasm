...
export default async function handler(req: NextRequest, event: Event): Promise<Response> {
    ...
    const importObject = {
        env: {
            ...
            render_move_in: (x, y) => {
                svgMazeAndSolution.push(`<animate xlink:href="#${x}_${y}" attributeName="fill" from="${BACKGROUND_COLOR}" to="${WALK_COLOR}" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`);
                tick += ANIMATION_DELAY;
            },
            render_move_out: (x, y) => {
                svgMazeAndSolution.push(`<animate xlink:href="#${x}_${y}" attributeName="fill" from="${WALK_COLOR}" to="${BACKGROUND_COLOR}" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`);
                tick += ANIMATION_DELAY;
            },
            render_solution: (x, y) => {
                    svgMazeAndSolution.push(`<animate xlink:href="#${x}_${y}" attributeName="fill" from="${WALK_COLOR}" to="${SOLUTION_COLOR}" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`);
                    tick += ANIMATION_DELAY;
            }
        },
    };
